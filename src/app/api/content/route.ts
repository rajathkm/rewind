  ...
  try {
    ...
    // The RPC function doesn't return audio_url field
    {
      ...
      const items = data?.map((item) => ({
        ...
        title: item.title,
        excerpt: item.excerpt || item.extracted_text?.slice(0, 200),
        url: item.url,
        imageUrl: item.image_url,
        // Audio fields - use audio_url for podcasts, media_url as fallback
        mediaUrl: item.audio_url || item.media_url,
        audioUrl: item.audio_url,
        publishedAt: item.published_at,
        // Duration - use audio_duration_seconds for podcasts, duration_seconds as fallback
        durationSeconds: item.audio_duration_seconds || item.duration_seconds,
        audioDurationSeconds: item.audio_duration_seconds,
